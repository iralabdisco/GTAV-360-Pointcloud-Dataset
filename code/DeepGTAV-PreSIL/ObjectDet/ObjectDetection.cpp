...
void ObjectDetection::initCollection(UINT camWidth, UINT camHeight, bool exportEVE, int startIndex) {
    ...
    m_pDepth = new float[(int)camWidth * (int)camHeight];
...
FrameObjectInfo ObjectDetection::generateMessage(float* pDepth, uint8_t* pStencil, int entityID, int sector) {
    ...
    setPosition();
    outputRealSpeed();
    if (lidar_initialized) setDepthBuffer();
...
void ObjectDetection::exportCameraPose() {
    Vector3 position = CAM::GET_CAM_COORD(camera);
    Vector3 rotation = CAM::GET_CAM_ROT(camera, 2);
    Vector3 posEntity, rightVec, forwardVec, upVec;
    ENTITY::GET_ENTITY_MATRIX(m_vehicle, &forwardVec, &rightVec, &upVec, &posEntity);

    drawVectorFromPosition(forwardVec, 0, 0, 255);
    drawVectorFromPosition(rightVec, 0, 255, 0);
    drawVectorFromPosition(upVec, 255, 0, 0);

    FILE* f = fopen(m_cameraPoseFilename.c_str(), "w");
    std::ostringstream oss;

    //oss << "posX" << "," << "posY" << "," << "posZ" << "," << "rotX" << "," << "rotY" << "," << "rotZ" << "\n";
    oss << position.x << "," << position.y << "," << position.z << "\n";
    oss << rotation.x << "," << rotation.y << "," << rotation.z << "\n";
    oss << forwardVec.x << "," << forwardVec.y << "," << forwardVec.z << "\n";
    oss << rightVec.x << "," << rightVec.y << "," << rightVec.z << "\n";
    oss << upVec.x << "," << upVec.y << "," << upVec.z << "\n";
...
void ObjectDetection::drawVectorFromPosition(Vector3 vector, int blue, int green, int red) {
    GRAPHICS::DRAW_LINE(s_camParams.pos.x, s_camParams.pos.y, s_camParams.pos.z, vector.x * 1000 + s_camParams.pos.x, vector.y * 1000 + s_camParams.pos.y, vector.z * 1000 + s_camParams.pos.z, red, green, blue, 200);
    WAIT(0);
}
...
void ObjectDetection::setFilenames(int sector) {
    //These are standard files
    switch (sector)
    {
    case 1: {
        m_imgFilename = getStandardFilename("image_1", ".png",3 );
        m_veloFilename = getStandardFilename("velodyne_1", ".bin", 1);
        m_depthFilename = getStandardFilename("depth_1", ".bin", 2);
        break;
    }
    case 2: {
        m_imgFilename = getStandardFilename("image_2", ".png", 3);
        m_veloFilename = getStandardFilename("velodyne_2", ".bin", 1);
        m_depthFilename = getStandardFilename("depth_2", ".bin", 2);
        break;
    }
    case 3: {
        m_imgFilename = getStandardFilename("image_3", ".png", 3);
        m_veloFilename = getStandardFilename("velodyne_3", ".bin", 1);
        m_depthFilename = getStandardFilename("depth_3", ".bin", 2);
        break;
    }
    case 4: {
        m_imgFilename = getStandardFilename("image_4", ".png", 3);
        m_veloFilename = getStandardFilename("velodyne_4", ".bin", 1);
        m_depthFilename = getStandardFilename("depth_4", ".bin", 2);
        break;
    }
    ...
    m_stencilFilename = getStandardFilename("stencil", ".raw", 0);
    m_labelsFilename = getStandardFilename("label_2", ".txt", 0);
    m_labelsAugFilename = getStandardFilename("label_aug_2", ".txt", 0);
    m_calibFilename = getStandardFilename("calib", ".txt", 0);
    m_cameraPoseFilename = getStandardFilename("camera_pose", ".txt", 0);

    //TODO - Why are two seg images being printed (there are some minor differences in images it appears)
    m_segImgFilename = getStandardFilename("segImage", ".png", 0);
    m_instSegFilename = getStandardFilename("instSeg", ".png", 0);
    m_instSegImgFilename = getStandardFilename("instSegImage", ".png", 0);

    //These files are for relating object positions to each other
    if (GENERATE_SECONDARY_PERSPECTIVES) {
        m_posFilename = getStandardFilename("position_world", ".txt", 0);
        m_egoObjectFilename = getStandardFilename("ego_object", ".txt", 0);
    }

    //The following are for testing, only create filenames/directories if they are set to be on
    if (OUTPUT_DM_POINTCLOUD) {
        m_depthPCFilename = getStandardFilename("depthPC", ".bin", 0);
        m_depthImgFilename = getStandardFilename("depthImage", ".png", 0);
    }
    if (OUTPUT_OFFSET_POINTCLOUDS) {
        m_veloFilenameU = getStandardFilename("velodyneU", ".bin", 0);
        m_depthPCFilenameU = getStandardFilename("depthPCU", ".bin", 0);
    }
    if (OUTPUT_GROUND_PIXELS) {
        m_groundPointsFilename = getStandardFilename("groundPointsImg", ".png", 0);
    }
    if (OUTPUT_UNUSED_PIXELS_IMAGE) m_unusedPixelsFilename = getStandardFilename("unusedPixelsImage", ".png", 0);
    if (OUTPUT_STENCIL_IMAGE) m_stencilImgFilename = getStandardFilename("stencilImage", ".png", 0);
    if (OUTPUT_OCCLUSION_IMAGE) m_occImgFilename = getStandardFilename("occlusionImage", ".png", 0);
    if (OUTPUT_UNPROCESSED_LABELS) m_labelsUnprocessedFilename = getStandardFilename("labelsUnprocessed", ".txt", 0);
}
...
void ObjectDetection::setupLiDAR() {
    if (pointclouds && !lidar_initialized) //flag if activate the LiDAR
    {
        ...
        //lidar.Init3DLiDAR_FOV(MAX_LIDAR_DIST, 90.0f, 0.09f, 26.9f, 0.420f, 15.0f);
...
void ObjectDetection::collectLiDAR() {
    ...
    float * pointCloud = lidar.GetPointClouds(pointCloudSize, &m_entitiesHit, lidar_param, m_pDepth, m_vehicle);
...
void ObjectDetection::setDepthBuffer() {
    int size = s_camParams.width * s_camParams.height;
    //log("About to set depth buffer");
    //log(std::to_string(size));
    
    FILE* ofile = fopen(m_depthFilename.c_str(), "wb");
    fwrite(m_pDepth, sizeof(float), size, ofile);
    fclose(ofile);
    scriptWait(10);

    log("Saving Depth buffer");
...
std::string ObjectDetection::getStandardFilename(std::string subDir, std::string extension, int type) {
    std::string filename;
